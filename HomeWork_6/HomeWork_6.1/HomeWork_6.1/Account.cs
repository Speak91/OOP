...
namespace HomeWork_6._1
{
    class Account
    {
        ...
        public void TransferFundsToCurrentAccount(Account account, int sum)
        {
            if (account._balance > sum)
            {
                account._balance -= sum;
                Console.WriteLine($"Перевод денежных средств на ваш счет со счета {account.AccountNumber} успешно произведен");
                DepositACheck(sum);
            }
            else
            {
        ...
        public static bool operator ==(Account acc1, Account acc2)
        {
            ...
            acc1.AccountType == acc2.AccountType)
            {
                return true;
            }
            else
        ...
        public bool Equals(Account acc1)
        {
           if (acc1.AccountNumber == this.AccountNumber &&
           acc1.Balance == this.Balance &&
           acc1.AccountType == this.AccountType)
            {
                return true;
            }
            else
